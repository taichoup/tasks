...
describe("requests API helpers", () => {
    ...
    it("fetchTasks returns parsed tasks", async () => {
        vi.mocked(fetch).mockResolvedValue(new Response(JSON.stringify([baseTask]), {
            status: 200,
            headers: { "Content-Type": "application/json" },
        }));

        await expect(fetchTasks()).resolves.toEqual([baseTask]);
        expect(fetch).toHaveBeenCalledWith(expect.stringMatching(/\/tasks$/), { method: "GET" });
    });

    it("addTask validates inputs before sending a request", async () => {
        await expect(addTask("   ", 1, "day", [])).rejects.toThrow("New title cannot be empty");
