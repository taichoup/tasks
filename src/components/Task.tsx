...
export const Task = ({ task }: TaskProps) => {
  ...
  const checkedAtDateDisplayString = task.checkedAt
    ? new Intl.DateTimeFormat("fr-FR", dateFormatOptions).format(
        new Date(task.checkedAt),
      )
    : null;
    ...
    <li key={task.id} title={`À refaire dans: ${timeRemainingUntilUncheck}`}>
      <div className={styles.taskDetails}>
        ...
        <span className={styles.taskDetailsItem}>
          {getLocalizedUnit(task.frequency?.unit, task.frequency.value)}
        </span>
        <span className={styles.taskDetailsItem}>
          {task.checkedAt ? `Fait depuis le ${checkedAtDateDisplayString}` : ""}
        </span>
        <span className={styles.taskDetailsItem}>
          {task.tags && task.tags.length > 0 ? (
            <Tag label={task.tags[0]} />
          ) : null}
        </span>
      </div>
      <button
