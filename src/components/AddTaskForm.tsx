...
export const AddTaskForm = () => {
  ...
  const [newRecurrenceUnit, setNewRecurrenceUnit] =
    useState<TaskFrequencyUnit>("day");
  const [newTags, setNewTags] = useState<TaskTags | []>([]);
  const queryClient = useQueryClient();

  const addTaskMutation = useMutation({
    mutationFn: () =>
      addTask(newTitle, newRecurrenceQty, newRecurrenceUnit, newTags),
    onSuccess: () => {
      setNewTitle("");
  ...
  return (
    <div className={styles.addTaskForm}>
      ...
      <select
        ...
        onChange={(e) =>
          setNewRecurrenceUnit(e.target.value as TaskFrequencyUnit)
        }
      >
