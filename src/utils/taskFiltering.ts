...
export type TaskTagFilter = typeof ALL_TAGS_FILTER | typeof UNTAGGED_FILTER | NonNullishTag;

export function getAvailableTaskTags(tasks: Task[]): NonNullishTag[] {
    return [...new Set(tasks.flatMap((task) => task.tags ?? []))].sort((a, b) => a.localeCompare(b, "fr"));
}

export function filterTasksByTag(tasks: Task[], selectedTag: TaskTagFilter): Task[] {
